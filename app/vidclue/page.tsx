...
export default function VidclueFinalPage() {
  ...
  return (
    <div className="relative min-h-screen w-full flex items-center justify-center p-2 md:p-4 font-sans select-none overflow-hidden bg-white">
      ...
      <div className="relative z-50 w-full h-auto md:max-w-[1100px] md:max-h-[95vh] md:aspect-[16/10] bg-[#bd4238] rounded-[20px] md:rounded-[40px] shadow-[0_60px_100px_rgba(0,0,0,0.6),inset_0_2px_4px_rgba(255,255,255,0.1)] p-3 md:p-6 flex flex-col transition-all duration-500 border border-black/20 pointer-events-auto">
        ...
        {/* Mobile: Power + Trusted By on top in compact format */}
        <div className="md:hidden relative z-[110] mb-2">
          <div className="bg-[#111]/85 border border-black/30 rounded-xl p-2 flex items-center gap-2">
            <button
              onClick={() => setIsOn(!isOn)}
              className="w-10 h-10 rounded-full bg-gradient-to-b from-[#3a3a3a] to-[#1a1a1a] border border-black/40 shadow-[0_4px_6px_rgba(0,0,0,0.4)] flex items-center justify-center pointer-events-auto shrink-0"
            >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between mb-1 px-1">
                <span className="text-[9px] font-black tracking-widest text-white/50 uppercase">Trusted by</span>
                <div className="flex gap-1">
                  {[0, 1, 2, 3].map(i => (
                    <div key={i} className={`w-1 h-1 rounded-full ${i === logoSlideIndex ? 'bg-yellow-400' : 'bg-white/15'}`} />
                  ))}
                </div>
              </div>
        ...
        {/* --- Main Interface Area --- */}
        <div className="flex-1 flex flex-col md:flex-row md:gap-5 relative z-[100] min-h-0 pointer-events-auto gap-2 md:gap-5">

          {/* Left Sidebar */}
          <div className={`flex-[0.9] w-full md:w-auto rounded-2xl overflow-hidden flex flex-col border-2 border-black/40 shadow-[inset_0_0_20px_rgba(0,0,0,0.5)] transition-colors duration-300 relative h-[360px] md:h-auto ${isOn ? 'bg-[#151515]' : 'bg-[#0d0d0d]'}`}>
            <div className={`h-full flex flex-col transition-opacity duration-300 relative z-0 ${isOn ? 'opacity-100' : 'opacity-20'}`}>
              ...
              <div className="mt-auto border-t border-white/10 bg-black/40 p-4 hidden md:block">
          ...
          <div className={`flex-1 w-full md:w-auto rounded-2xl overflow-hidden flex flex-col border-2 border-black/40 shadow-[inset_0_0_20px_rgba(0,0,0,0.2)] transition-colors duration-300 relative h-[360px] md:h-auto ${isOn ? 'bg-[#e0e0e0]' : 'bg-[#1a1a1a]'}`}>
        ...
        <div className="flex md:hidden flex-col gap-2 w-full">
          ...
          <div className="flex gap-2 items-center justify-between bg-[#1a1a1a] rounded-lg p-2">
            ...
            <div className="flex items-center gap-2 bg-[#121212] rounded-lg p-1.5 border border-white/5">
              ...
              <span className="text-[10px] font-bold text-white/60 min-w-[52px] text-center">{currentIndex + 1}/{items.length}</span>
